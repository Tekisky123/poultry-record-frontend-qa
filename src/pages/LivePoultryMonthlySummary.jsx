...
export default function LivePoultryMonthlySummary() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (res.data.success) {
                // Get ALL bird and feed stocks to calculate running balances
    ...
    const monthlyData = useMemo(() => {
        ...
        sortedMonths.forEach(month => {
            const birdPrevAvgRate = cumulativeBirdPurchWeight > 0 ? (cumulativeBirdPurchAmount / cumulativeBirdPurchWeight) : 0;
            const currentBirdOpeningWeight = cumulativeBirdPurchWeight - cumulativeBirdOutWeight;
            const currentBirdOpeningAmount = currentBirdOpeningWeight * birdPrevAvgRate;

            const feedPrevAvgRate = cumulativeFeedPurchWeight > 0 ? (cumulativeFeedPurchAmount / cumulativeFeedPurchWeight) : 0;
            const currentFeedOpeningWeight = cumulativeFeedPurchWeight - cumulativeFeedOutWeight;
            const currentFeedOpeningAmount = currentFeedOpeningWeight * feedPrevAvgRate;

            month.birdsWeight = currentBirdOpeningWeight;
            month.birdsAmount = currentBirdOpeningAmount;
            
            month.feedWeight = currentFeedOpeningWeight;
            month.feedAmount = currentFeedOpeningAmount;
            
            month.totalAmount = currentBirdOpeningAmount + currentFeedOpeningAmount;

            const monthObj = new Date(Date.parse(month.name + " 1, 2012")).getMonth();
            const monthlyStocks = stocksDuringFY.filter(s => {
                const d = new Date(s.date);
                return d.getFullYear() === month.year && d.getMonth() === monthObj;
            });
    ...
    const handleMonthClick = (month) => {
        ...
        navigate(`/live-poultry-stock/daily-summary?year=${month.year}&month=${monthNum}`);
    };
    ...
    const handleExportToExcel = () => {
        ...
        exportData.push({
            'Month': 'Total',
            'Birds Qty (kg)': totals.birdsW,
            'Birds Rate': totals.birdsW ? (totals.birdsA / totals.birdsW).toFixed(2) : '0.00',
            'Birds Amount': totals.birdsA,
            'Feed Qty (kg)': totals.feedW,
            'Feed Rate': totals.feedW ? (totals.feedA / totals.feedW).toFixed(2) : '0.00',
            'Feed Amount': totals.feedA,
            'Total Amount': totals.totalA
        });

        const ws = XLSX.utils.json_to_sheet(exportData);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Opening Stock Monthly");
        XLSX.writeFile(wb, `Live_Poultry_Opening_Stock_${year}-${year + 1}.xlsx`);
    };
                ...
                <div>
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-2">
                            ...
                            Live Poultry Opening Stock (Monthly)
                        </h1>
                    </div>
                    <p className="text-gray-600 mt-1">Monthly Opening Records for Birds and Feed Stock</p>
                </div>
                ...
                <table className="w-full text-sm text-left">
                    <thead className="bg-gray-100 text-gray-700 uppercase font-semibold border-b-2 border-gray-300">
                        <tr>
                            ...
                            <th colSpan="3" className="py-2 px-4 text-center border-r border-gray-300 bg-orange-50 text-orange-800">Birds Opening Stock</th>
                            <th colSpan="3" className="py-2 px-4 text-center border-r border-gray-300 bg-yellow-50 text-yellow-800">Feed Opening Stock</th>
                            <th rowSpan="2" className="py-3 px-4 text-center bg-blue-50 text-blue-800">Total Amount</th>
                        </tr>
                        <tr className="border-t border-gray-300">
                            <th className="py-2 px-4 bg-orange-50">Quantity (kg)</th>
                            <th className="py-2 px-4 bg-orange-50">Rate</th>
                            <th className="py-2 px-4 border-r border-gray-300 bg-orange-50">Amount</th>
                            <th className="py-2 px-4 bg-yellow-50">Quantity (kg)</th>
                            <th className="py-2 px-4 bg-yellow-50">Rate</th>
                            <th className="py-2 px-4 border-r border-gray-300 bg-yellow-50">Amount</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200">
                        {monthlyData.map((month) => (
                            ...
                            >
                                ...
                                <td className="py-3 px-4 text-right text-orange-700 font-medium">{month.birdsWeight ? month.birdsWeight.toLocaleString('en-IN', { minimumFractionDigits: 2 }) : '-'}</td>
                                ...
                                <td className="py-3 px-4 text-right text-yellow-700 font-medium">{month.feedWeight ? month.feedWeight.toLocaleString('en-IN', { minimumFractionDigits: 2 }) : '-'}</td>
                    ...
                    <tfoot className="bg-gray-100 font-bold text-gray-900 border-t-2 border-gray-400">
                        <tr>
                            ...
                            <td className="py-3 px-4 text-right text-orange-700">{totals.birdsW.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                            ...
                            <td className="py-3 px-4 text-right text-yellow-700">{totals.feedW.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
