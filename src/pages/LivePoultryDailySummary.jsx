...
export default function LivePoultryDailySummary() {
    ...
    const dailyData = useMemo(() => {
        ...
        sortedDays.forEach(day => {
            ...
            day.birdsWeight = currentBirdOpeningWeight;
            day.birdsAmount = currentBirdOpeningAmount;
            
            day.feedWeight = currentFeedOpeningWeight;
            day.feedAmount = currentFeedOpeningAmount;
            
            day.totalAmount = currentBirdOpeningAmount + currentFeedOpeningAmount;
        ...
        sortedDays.forEach(day => {
            day.birdsRate = day.birdsWeight ? day.birdsAmount / day.birdsWeight : 0;
            day.feedRate = day.feedWeight ? day.feedAmount / day.feedWeight : 0;
        });
    ...
    const handleExportToExcel = () => {
        ...
        const exportData = dailyData.map(day => ({
            'Date': day.dayKey,
            'Birds Qty (kg)': day.birdsWeight,
            'Birds Rate': day.birdsRate.toFixed(2),
            'Birds Amount': day.birdsAmount,
            'Feed Qty (kg)': day.feedWeight,
            'Feed Rate': day.feedRate.toFixed(2),
            'Feed Amount': day.feedAmount,
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Opening Stock Daily");
        XLSX.writeFile(wb, `Live_Poultry_Opening_Stock_${year}-${String(month).padStart(2, '0')}.xlsx`);
    };
                ...
                <div>
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-2">
                            ...
                            Live Poultry Opening Stock (Daily)
                        </h1>
                    </div>
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {dailyData.map((day) => (
                            <tr key={day.dayKey} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="py-3 px-4 text-right text-orange-700 font-medium">{day.birdsWeight ? day.birdsWeight.toLocaleString('en-IN', { minimumFractionDigits: 2 }) : '-'}</td>
                                ...
                                <td className="py-3 px-4 text-right text-yellow-700 font-medium">{day.feedWeight ? day.feedWeight.toLocaleString('en-IN', { minimumFractionDigits: 2 }) : '-'}</td>
