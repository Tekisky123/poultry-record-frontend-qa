...
export default function VendorMonthlySummary() {
    const { id } = useParams();
    ...
    const fetchMonthlySummary = async () => {
        try {
            setLoading(true);
            setError('');
            // Use ledger controller endpoint which handles vendors too
            const response = await api.get(`/ledger/${id}/monthly-summary`, { params: { year, type: 'vendor' } });
    ...
    const handleExportToExcel = () => {
        ...
        const exportData = data.months.map(m => ({
            Month: m.name,
            'Total Debit (Paid)': m.debit,
            'Total Credit (Purchased)': m.credit,
